...
InventoryUI.Container:GetPropertyChangedSignal("Position"):Connect(function()
    hoverZone.Position = InventoryUI.Container.Position
end)
    ...
    if Holder:FindFirstChild(name) then
        ...
        self.Connections = {
            ...
            UserInputService.InputEnded:Connect(function(input)
                if not self.IsDragging or input.UserInputType ~= Enum.UserInputType.MouseButton1 then
                    return
                end
                ...
                if self:IsDraggingOverInventory(input) then
                    ...
                    if self.Stack > 1 then
                        ...
                        workspace:SetAttribute("TargetItem", self.Name)
                    else
...
function Item:SetPosition(position: number)
    ...
    self.Slot:TweenPosition(UDim2.fromOffset(x, y), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.1, true)
end
